...
test('createDefaultSettings merges overrides', () => {
  const settings = createDefaultSettings({
    dailyCap: 5,
    sessions: {
      chatgpt: { profilePath: '/tmp/chatgpt-profile', cookiesEncrypted: true }
    },
    consentFlags: { storeCookies: false }
  });
  assert.equal(settings.dailyCap, 5);
  assert.equal(settings.targetLLM, 'chatgpt');
  assert.ok(settings.llmWebTargets.includes('https://chat.openai.com'));
  assert.equal(settings.sessions.chatgpt.profilePath, '/tmp/chatgpt-profile');
  assert.equal(settings.consentFlags.storeCookies, false);
});
